...
///  Vector prior mean
///  Multi company Model
///  Distance score for each company
...
functions {
  ...
  matrix cov_categorical(int[] c1, int[] c2, vector rho_company){
    int n1 = size(c1);
    int n2 = size(c2);
    ...
    for (i1 in 1:n1){
      for (i2 in 1:n2){
        if(c1[i1] == c2[i2]){
          ...
          cat_mat[i1,i2] = -rho_company[c1[i1]] - rho_company[c2[i2]];
        }
      }
    }
  ...
  matrix cov_fun(vector[] x1, vector[] x2, int[] x_cat1, int[] x_cat2, vector[] h1, vector[] h2,   
    real eta, vector rho, vector theta, vector rho_company, real delta, int no_input_vectors){
      ...
      res = res .* cov_categorical(x_cat1, x_cat2, rho_company);
  ...
  vector gp_pred_rng(vector[] x1, vector[] x2, int[] x_cat1, int[] x_cat2, vector[] h1, vector[] h2, int[] dev_lag,
                     ...
                     real eta, vector rho, vector sigma, vector theta, vector rho_company,
                     real delta) {
    int N1 = rows(y1);
    int N2 = size(x2);
    vector[N2] f2;
    {
      matrix[N1, N1] K =   add_sigma(cov_fun(x1, x1, x_cat1, x_cat1, h1, h1, eta, rho, theta,  rho_company, delta, 1), 
                                      square(sigma),dev_lag);
      matrix[N1, N1] L_K = cholesky_decompose(K);

      vector[N1] L_K_div_y1 = mdivide_left_tri_low(L_K, y1-mu_vec1);
      vector[N1] K_div_y1 = mdivide_right_tri_low(L_K_div_y1', L_K)';
      matrix[N1, N2] k_x1_x2 = cov_fun(x1, x2, x_cat1, x_cat2, h1, h2, eta, rho, theta, rho_company, 0, 2);
      vector[N2] f2_mu = mu_vec2 + (k_x1_x2' * K_div_y1);
      matrix[N1, N2] v_pred = mdivide_left_tri_low(L_K, k_x1_x2);
      matrix[N2, N2] cov_f2 = cov_fun(x2, x2, x_cat2, x_cat2, h2, h2, eta, rho, theta, rho_company, delta,1) -v_pred' * v_pred;
...
data {
  ...
  int<lower=1> noCompanies; // number of companies
 ...
 real<lower=0> prior_rho_company;
...
parameters {
  ...
  vector<lower=0>[noCompanies] rho_company;
  vector[N1] f;
}

transformed parameters {
  vector<lower=0>[D] rho_sq;
  //vector<lower=0>[noCompanies] rho_company_sq;
  real<lower=0> eta_sq;
  ...
  // for (i in 1:noCompanies){
  //   rho_company_sq[i] = rho_company[i]^2;
  // }
...
model {
  ...
  // Mean field function
  matrix[N1, N1] cov =   cov_fun(x1, x1, x_cat1, x_cat1, H1_pos, H1_pos,  eta, rho, theta,  rho_company, delta, 1);
  matrix[N1, N1] L_cov;
  vector[N1] hurdle;
  ...
  rho_company ~ normal(0, prior_rho_company);
...
generated quantities {
  vector[N2] fStar = gp_pred_rng(x1, x2, x_cat1, x_cat2, H1_pos, H2_pos, dev_lag1, y1, mu_vec1, mu_vec2, 
                                 eta, rho, sigma, theta, rho_company, delta);
  vector[N2] yStar;
  for (n in 1:N2)
